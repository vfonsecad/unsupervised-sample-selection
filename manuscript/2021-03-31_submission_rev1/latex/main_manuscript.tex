...
In the current study, we presented a generalization of the sample size and strategy for sample selection under the assumption of the validity of a PLSR model for the NIR application at hand. Model architectures such as PCR may also result valid in which case the problem of unsupervised sample selection is directly solved by Optimal Designs of Experiments, of which D-optimal is a particular case, using the PC scores. To this extent, if the subset of samples is selected under the current guideline with a D-optimal design, the selected set is representative for a PLSR model and optimal for a PCR model. In regard of other linear model architectures, such as ridge regression or similar, a subset of samples chosen under the current guideline and criterion of the $\mathbf{S}$ matrix is also representative given that those model architectures also depend on the covariance matrix $\mathbf{S}$ as the mathematical element that does not rely on $y$.
