...
\textcolor{blue}{\subsection{About the sample size}}
...
\textcolor{blue}{\subsection{About the representativity of the samples}}
...
\textcolor{blue}{\subsection{About preprocessing}}
...
However, initial experiments in the current case studies suggested that assuming advanced preprocessing such as scatter correction resulted in sets rendering calibration models with poor performance. This suggests that strong assumptions related to the required preprocessing prior to obtaining reference analyses are not recommended. \textcolor{blue}{In the context of NIR applications and PLSR models, it has also been reported that some preprocessing methods may result in undesired effects for the calibration model and that it is also possible that the model can cope with some of those effects given a sufficient sample size \cite{Schoot2020}. These facts motivated the exclusion of the effect of preprocessing for unsupervised sample selection in the current study, leaving open the possibility for further study and applications. }

\textcolor{blue}{\subsection{About other bilinear and linear models}}

\textcolor{blue}{In the current study, we presented a generalization of the sample size and strategy for sample selection under the assumption of the validity of a PLSR model for the NIR application at hand. Model architectures such as PCR may also result valid in which case the problem of unsupervised sample selection is directly solved by Optimal Designs of Experiments, of which D-optimal is a particular case, using the PC scores. To this extent, if the subset of samples is selected under the current guideline with a D-optimal design, the selected set is representative for a PLSR model and optimal for a PCR model. In regard of other linear model architectures, such as ridge regression or similar, a subset of samples chosen under the current guideline and equivalence criterion of the $\mathbf{S}$ matrices is also representative given that such model architectures also depend on the covariance matrix $\mathbf{S}$ as the mathematical element that does not rely on $y$.}
