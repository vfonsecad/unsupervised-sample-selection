...
% ------------------- authors  ------------
% -----------------------------------------------------
\author[mebios]{Valeria Fonseca Diaz\corref{mycorrespondingauthor}}
\cortext[mycorrespondingauthor]{Corresponding author}
\ead[url]{biw.kuleuven.be/biosyst/mebios/biophotonics-group}
\ead[mail]{valeria.fonsecadiaz@kuleuven.be}
...
\begin{keyword}
sample selection \sep unsupervised learning \sep multivariate calibration \sep PLS regression \sep VC dimension \sep NIR
...
It is in this regard that the problem stated in eq. (\ref{eq_square_loss_empirical_regression_problem}) corresponds to the definition of the PLSR model  \cite{Stone1990}. More concretely, the SIMPLS or NIPALS algorithms construct an orthogonal set of latent variables by maximizing the covariance between $y$ and $\mathbf{x}$ which corresponds to the basis $\{\phi_{k}(\mathbf{x})\}$ and then a least squares regression is performed between the set of these latent variables $\{\phi_{k}(\mathbf{x})\}$ and $y$. The order in this basis is established by the covariance deflation at each step of the PLSR algorithm. The key element in this framework is the fact that the number of chosen basis functions $d$ corresponds to the so-called $VC$ dimension in Vapnik's theory, which measures the capacity control of a learning machine \cite{Vapnik2019}. The importance of the capacity control parameter $d$ is that it serves as the reference for determining a suitable sample size when aiming to build a regression machine, or, in the context of chemometrics, a multivariate calibration model. In the work by Vapnik, it has been stated that the ratio between the sample size and the $VC$ dimension determines whether the sample size is large or small. As the sample size becomes large, the \emph{empirical} risk becomes closer to the \emph{expected} risk \cite{Vapnik2000}. Although there is no absolute threshold, it has been stated that the sample size is considered \emph{large} when  $n/d>20$ \cite{Vapnik2000}.


\subsection{Bilinear modelling as a specific framework}

In bilinear modelling, the basis of $L_2$ is what is known as the set of latent variables. Based on a set of $n$ observations stored in the matrices $\mathbf{X}_{n\times p}$ and $Y_{n\times 1}$, PLSR calculates latent variables $\{\phi_{k}(\mathbf{x})\}$ such that $\phi_k(\mathbf{x}) = \mathbf{Xv}_{k}$, where $\mathbf{v}_k$ results from maximizing the covariance between $\mathbf{X}$ and $Y$ at the $k$-th deflation step \cite{DeJong1993}. 

For a given value of $d$, the set of the resulting $d$ latent variables constitute a set of orthonormal variables $\{\phi_{k}(\mathbf{x})\}_{i=1}^d$. However, during the maximization of $\mathbf{s}=$ cov($\mathbf{X}$, $Y$), the set of loading vectors $\{\mathbf{v}_k\}$ is defined as $\mathbf{S}$-orthogonal as it holds true that $\mathbf{v}_k'\mathbf{S}\mathbf{v}_j = 0 \quad (j<k)$, where $\mathbf{S}=$ cov($\mathbf{X}$) is the covariance matrix of $\mathbf{X}$. Therefore, while PLSR builds a set of latent variables by maximizing $\mathbf{s}$ as defined by SIMPLS or NIPALS, it uses $\mathbf{S}=$ cov($\mathbf{X}$) in the process, which is an unsupervised element of this model and algorithm.

To define a criterion to select representative samples for PLSR models, this so-called $\mathbf{S}$-orthogonality definition of the SIMPLS or NIPALS algorithms suggests that if $n<N$ samples are found as $\mathbf{X}_{n \times p} \subset \mathbf{X}_{N \times p}$ such that $\mathbf{S}_n$ and $\mathbf{S}_N$ are equivalent, the selection method would not discard unsupervised information that is crucial for building the PLSR model by discarding the $N-n$ remaining samples, selecting the $n$ most informative samples.
...
\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{d01_milk_general_framework.png}
\centering
...
\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{d02_manure_general_framework.png}
\centering
...
\begin{figure}[H]
    \centering
    \subfigure[Milk]{\includegraphics[width=0.8\textwidth]{d01_milk_specific_framework_detereigevect.png}} 
    \subfigure[Manure]{\includegraphics[width=0.8\textwidth]{d02_manure_specific_framework_detereigevect.png}}
...
\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{d01_milk_specific_framework_eigenvalsratio.png}
\centering
...
\begin{figure}[H]
\includegraphics[width=0.9\textwidth]{d02_manure_specific_framework_eigenvalsratio.png}
\centering
...
\begin{figure}[H]
\includegraphics[width=1\textwidth]{d01_milk_model_performance.png}
\centering
...
\begin{figure}[H]
\includegraphics[width=1\textwidth]{d02_manure_model_performance.png}
\centering
...
\subsection{About the representativity of the samples}

The leading feature to address the current problem is that of spanning as much as possible the variability contained in the original set so that a representative subset of samples is obtained. In the context of chemometrics with PLSR models, this feature had not been concretely translated into a mathematical criterion, under the assumption that PLSR can model the relationship between $\mathbf{X}$ and $y$. The specific framework analysis showed the role that the matrix $\mathbf{S} =$ cov($\mathbf{X}$) plays in the PLSR model, making it the mathematical element that can be evaluated in an unsupervised setting. Therefore, finding a subset of $n$ samples that renders $\mathbf{S}_n$ equivalent to $\mathbf{S}_N$ has been proposed here as a way to define the representativity of the selected samples for a PLSR model.
...
However, initial experiments in the current case studies suggested that assuming advanced preprocessing such as scatter correction resulted in sets rendering calibration models with poor performance. This suggests that strong assumptions related to the required preprocessing prior to obtaining reference analyses are not recommended. In the context of NIR applications and PLSR models, it has also been reported that some preprocessing methods may result in undesired effects for the calibration model and that the model may cope with some of those effects given a sufficient sample size \cite{Schoot2020}. These facts motivated the exclusion of the effect of preprocessing for unsupervised sample selection in the current study. Nonetheless, if the optimal preprocessing is well known a priori, the sample selection may be performed with preprocessed data.

\subsection{About other bilinear and linear models}

In the current study, we presented a generalization of the sample size and strategy for sample selection under the assumption of the validity of a PLSR model for the application at hand. While a PCR model will typically require a large number of components $d$ than the corresponding PLSR model, the problem of unsupervised sample selection for PCR is directly solved by Optimal Designs of Experiments, of which D-optimal is a particular case, using the PC scores. If the PCR model requires more components, this optimal set would contain more samples than a representative set for PLSR.

In regard of other linear model architectures, such as ridge regression or similar, a subset of samples chosen under the current guideline and equivalence criterion of the $\mathbf{S}$ matrices is also representative given that such model architectures also depend on the covariance matrix $\mathbf{S}$ as the mathematical element that does not rely on $y$.
...
The criterion based on the equivalence between $\mathbf{S}_N$ and $\mathbf{S}_n$ is specific for bilinear models such as PLSR, PLS discriminant analysis, linear regression, among others. Further research is recommended to refine this criterion of equivalence and to propose a sample selection algorithm aimed at optimizing this equivalence. This concept could be extended to other model architectures by identifying the mathematical components that could be controlled or evaluated in unsupervised sample selection \cite{Li2020}. 
